...
public:
		...
		,idAttribute_names(idAttribute_names)
	{
		...
		//TODO: Add check that all the required tables of "SQL_RequestList_Type" are in "table_names"
	}

	//Create a class for reading from a single table
	SQLite3_Helper_TableRead(const std::string& table_name)
		:SQLite3_Helper_TableRead(std::make_tuple(table_name), std::make_tuple())
	{}
